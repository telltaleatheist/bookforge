...
// which is what cli/blocks-classify.js does, so batch classification runs THIS
 ...
 * kept honest by a golden fixture — `blocks-encoder.spec.ts` replays real
...
/**
 * A foundry blocks RELEASE → the PROMPT VERSION its weights were trained on.
 *
 * These two numbers came apart when the model moved into foundry (Aug 3 2026).
 * The old ids were `rubric-v5-4b`, where the version WAS the prompt format, so
 * reading it out of the name was safe. `foundry-blocks-v1-4b` is release 1 of a
 * stage — and release 1 ships the v5 prompt, because it is the v5 checkpoint
 * re-published under the new name. Parsing `v1` out of that id would hand a
 * twelve-class model the retired sixteen-class prompt, which does not error; it
 * just scores worse and reads like an undertrained model.
 *
 * The authority for a SHIPPING model is the catalog (`BlocksModelDef.promptVersion`
 * in electron/blocks-models.ts, mirroring foundry's own `FoundryModelDef`), and
 * Detect is handed that number over IPC. This table exists for the callers that
 * have no catalog to ask — the CLI tools, which drive the encoder directly. Both
 * must agree. A release that is not listed THROWS rather than guessing.
 */
const FOUNDRY_BLOCKS_PROMPT_VERSION: Record<number, BlocksVersion> = {
  1: 5,
};

/**
 * Which prompt format a model NAME implies.
 *
 * Names are all some runtimes report back — blocks-serve returns its adapter
 * path, Ollama returns the model name — so for those the convention is
 * load-bearing: put the version in the name (`blocks-v3-0.6b`) and it does not
 * matter what size or base the model is.
 *
 * A foundry id is the exception and is NOT parsed: its version segment is the
 * release, not the prompt, so it is looked up in the table above and an unknown
 * release is an error. Prefer the catalog's declared `promptVersion` wherever one
 * is available — this function is the answer of last resort.
 */
export function blocksVersionFor(name: string): BlocksVersion {
  const foundry = /^foundry-blocks-v(\d+)\b/i.exec(name);
  if (foundry) {
    const release = Number(foundry[1]);
    const version = FOUNDRY_BLOCKS_PROMPT_VERSION[release];
    if (!version) {
      throw new Error(
        `No prompt version declared for foundry blocks release v${release} ("${name}"). `
        + 'A foundry id names a RELEASE, not a prompt format — add it to '
        + 'FOUNDRY_BLOCKS_PROMPT_VERSION in blocks-encoder.ts (and to promptVersion in '
        + 'electron/blocks-models.ts) before shipping it.');
    }
    return version;
  }
