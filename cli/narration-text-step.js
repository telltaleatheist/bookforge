...
async function runNarrationTextStep(inputPath, opts) {
  ...
  if (fs.existsSync(wanted)) {
    const existing = readReceipt(receiptPathFor(wanted));
    if (reusable(existing, inputSha16, normalizer.NORMALIZER_VERSION,
      punctuation.PUNCTUATION_SPEC_VERSION)) {
      console.log(`[narration-text] reusing the cleaned book beside the input: ${wanted}`);
      return {
        inputPath: wanted, receiptPath: receiptPathFor(wanted), receipt: existing, ran: false,
      };
    }
  }
