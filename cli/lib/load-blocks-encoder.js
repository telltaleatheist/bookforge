/**
 * Load the blocks encoder FROM SOURCE, every run.
 *
 * The encoder owns the prompt format and the legal class list, and
 * `blocksVersionFor()` picks both by reading the version out of the model id.
 ...
 * `dist/blocks/.../blocks-encoder.js`, existence-checked and never
...
const SOURCE = path.join(REPO_ROOT, 'src/app/features/pdf-picker/services/blocks-encoder.ts');

let cached = null;

/** The encoder module, built from the current source. */
function loadBlocksEncoder() {
  if (cached) return cached;

  if (!fs.existsSync(SOURCE)) {
    console.error(`blocks: the encoder source is missing (${SOURCE}).`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('blocks: esbuild is not installed — run npm install.');
    process.exit(1);
  }
  ...
  const temp = path.join(os.tmpdir(), `bookforge-blocks-encoder-${process.pid}.cjs`);
  ...
  if (typeof cached.blocksVersionFor !== 'function') {
    console.error('blocks: the encoder bundle exports no blocksVersionFor — it was restructured.');
    process.exit(1);
  }
  return cached;
}

module.exports = { loadBlocksEncoder };
