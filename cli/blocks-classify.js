#!/usr/bin/env node
/**
 * blocks-classify — run the block-category model over a book's OCR blocks.
 *
 *   node --require cli/electron-stub.js cli/blocks-classify.js <blocks.json> \
 *        --out <predictions.json> [--model blocks-v3-4b] [--endpoint URL]
 ...
 *
 *   prompt building   src/app/features/pdf-picker/services/blocks-encoder.ts
 ...
 *   inference         electron/blocks-bridge.ts (`blocksClassify`), which
 ...
 *
 * The model name is LOAD-BEARING: `blocksVersionFor()` reads v1/v2/v3 out of
...
const BRIDGE = path.join(REPO_ROOT, 'dist', 'electron', 'blocks-bridge.js');

function usage(msg) {
  if (msg) console.error(`blocks-classify: ${msg}`);
  console.error(
    'usage: node --require cli/electron-stub.js cli/blocks-classify.js <blocks.json>\n' +
...
function requireBuilt(p, what, how) {
  if (!fs.existsSync(p)) {
    console.error(`blocks-classify: ${what} is not built (${p}).\n         ${how}`);
    process.exit(1);
  }
  return require(p);
}
...
const { blocksClassify } = requireBuilt(BRIDGE, 'the blocks bridge',
...
async function main() {
  ...
  if (!Array.isArray(doc.blocks) || !doc.blocks.length) {
    console.error('blocks-classify: blocks.json has no blocks');
    process.exit(1);
  }
  ...
  if (!encoded.length) {
    console.error('blocks-classify: no pages to classify after filtering');
    process.exit(1);
  }

  console.log(`[blocks] ${encoded.length} pages, ${blocks.length} blocks, ` +
  ...
  for (let i = 0; i < encoded.length; i += batch) {
    ...
    process.stderr.write(`\r[blocks] ${done}/${encoded.length} pages  ` +
  ...
  console.log(`[blocks] predicted ${Object.keys(predictions).length}/${total} blocks` +
    (unparsed ? ` (${unparsed} unlabelled — dropped rather than guessed)` : '') +
    ` in ${((Date.now() - started) / 1000).toFixed(1)}s`);
  console.log('[blocks] ' + Object.entries(counts).sort((a, b) => b[1] - a[1])
    .map(([c, n]) => `${c} ${n}`).join('  '));
  console.log(`[blocks] wrote ${outPath}`);
}

main().catch((err) => {
  console.error('\nblocks-classify failed:', err && err.stack ? err.stack : err);
  process.exit(1);
});
