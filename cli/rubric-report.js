#!/usr/bin/env node
/**
 * rubric-report — what did the model get wrong, and where should labelling go next.
 *
 *   node --require cli/electron-stub.js cli/rubric-report.js --project <dir> [--json out.json]
 *
 * Reads the stored Detect run (`editor.rubricPredictions`, written by
 * cli/rubric-detect.js) and the corrections a human made after it
 ...
 *
 * Read the numbers against the known state (docs/RUBRIC_TRAINING.md): a class is
...
function usage(msg) {
  if (msg) console.error(`rubric-report: ${msg}`);
  console.error('usage: node --require cli/electron-stub.js cli/rubric-report.js --project <dir> [--json out.json]');
...
const storePath = path.join(REPO_ROOT, 'dist', 'electron', 'rubric-predictions.js');
if (!fs.existsSync(storePath)) {
  console.error(`rubric-report: the prediction store is not built (${storePath}).\n` +
    '         Build the main process with:  npm run build:electron');
  process.exit(1);
}
const { buildRubricErrorReport } = require(storePath);

const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));
let r;
try {
  r = buildRubricErrorReport(manifest);
} catch (err) {
  console.error(`rubric-report: ${err.message}`);
  process.exit(1);
}
