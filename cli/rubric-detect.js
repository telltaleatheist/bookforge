#!/usr/bin/env node
/**
 * rubric-detect — run the page-layout model over a project and PERSIST the answer.
 *
 *   node --require cli/electron-stub.js cli/rubric-detect.js --project <dir> \
 *        [--model rubric-v3-4b] [--backend local|ollama|service] [--batch 8]
 *        [--pages a-b] [--dry-run]
 *
 * The difference from `rubric-classify` is where the answer goes. classify reads a
 ...
 * AND a measurement of where the model fails. `cli/rubric-report.js` recovers the
 ...
 *   prompt      src/app/features/pdf-picker/services/rubric-encoder.ts
 *   inference   electron/rubric-bridge.ts
 *   persistence electron/rubric-predictions.ts
...
function usage(msg) {
  if (msg) console.error(`rubric-detect: ${msg}`);
  console.error(
    'usage: node --require cli/electron-stub.js cli/rubric-detect.js --project <dir>\n' +
    '         [--model NAME] [--backend local|ollama|service] [--batch 8]\n' +
    '         [--pages a-b] [--dry-run]\n\n' +
    '  Writes editor.rubricPredictions and paints editor.ocrBlocks[].category_id.\n' +
...
function requireBuilt(rel, what, how) {
  ...
  if (!fs.existsSync(p)) {
    console.error(`rubric-detect: ${what} is not built (${p}).\n         ${how}`);
    process.exit(1);
  }
  return require(p);
}
...
async function main() {
  ...
  if (!pdf) {
    console.error(`rubric-detect: no PDF in ${path.join(ref.projectDir, 'archive')} — cannot read page sizes`);
    process.exit(1);
  }
  ...
  if (!pageDimensions.length) {
    console.error('rubric-detect: the analyzer reported no page dimensions');
    process.exit(1);
  }
  ...
  if (!encoded.length) { console.error('rubric-detect: no pages to classify'); process.exit(1); }
  ...
  for (let i = 0; i < encoded.length; i += batch) {
    ...
    if (!res.success) { console.error(`\nrubric-detect: ${res.error}`); process.exit(1); }
    slice.forEach((p, k) => {
  ...
  console.log(`[detect]   node --require cli/electron-stub.js cli/rubric-report.js --project ${projectDir}`);
}

main().catch((err) => {
  console.error('\nrubric-detect failed:', err && err.stack ? err.stack : err);
  process.exit(1);
});
