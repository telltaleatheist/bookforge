...
// Voice clips are PULLED at runtime now (not bundled): the default voice via the
// 'default-voice' runtime asset, the Voice Library via 'library-voices', and
// catalog voices fetch their clip with the model. So a normal build ships no
// voice clips at all — see the voices/ handling in the snapshot filter below.
// Only a full offline (--models) build still bundles them.
...
fs.cpSync(e2aSource, snapshotDest, {
  ...
  filter: (src) => {
    ...
    if (parts[0] === 'voices') {
      // Voice clips are PULLED at runtime now, not bundled: catalog voices fetch
      // their clip with the model, the default voice via the 'default-voice'
      // runtime asset, and the Voice Library clips via the 'library-voices' asset
      // (electron/e2a-env-bootstrap.ts). So a normal build ships NO voice clips —
      // it keeps the installer ~120 MB lighter. Only a full offline (--models)
      // build still bundles them (it can't download anything at first run).
      if (!includeModels) return false;
      if (seedModels) {
