#!/usr/bin/env node
/**
 * publish-release.js — publish a managed binary or the starter library.
 *
 * It updates `releases.json` (the data the catalog indexer merges into manifest.json),
 * then prints the exact commands to (1) upload the artifact to GitHub Releases and
 * (2) commit releases.json + trigger the catalog-indexer GitHub Action that
 ...
 * (requires the `gh` CLI). The manifest-refresh step is always printed.
 *
 ...
 *
 * Usage (starter library — the one-time first-run sample project):
 *   node packaging/publish-release.js --starter [--starter-name <slug>] [--publish]
 *   Prereq: node packaging/build-starter-library.js --name <slug>   (produces the sidecar + tarball)
 *
 * The app code bundle mode is GONE — the code-bundle self-update launcher was removed
 * (the app is a normal packaged Electron app now; ship updates by shipping installers).
 *
...
// releases.json, versioned in the repo — it IS the indexer's input. Commit + push it and the
// catalog-indexer GitHub Action merges it with upstream voices/languages into manifest.json.
const RELEASES_PATH = path.join(ROOT, 'tools', 'catalog-indexer', 'releases.json');

// The repo whose catalog-indexer Action regenerates manifest.json.
const GH_REPO = process.env.BOOKFORGE_GH_REPO || 'telltaleatheist/bookforge';
...
function printDeploy() {
  const rel = path.relative(ROOT, RELEASES_PATH);
  console.log('\n[publish] refresh manifest.json (regenerated by the catalog-indexer GitHub Action):');
  console.log(`  git add ${rel} && git commit -m "release: bump" && git push`);
  console.log(`  gh workflow run catalog-indexer.yml --repo ${GH_REPO}`);
  console.log('  (or just wait for the daily run; it merges releases.json + upstream voices/languages →');
  console.log('   manifest.json on the catalog-data branch)');
}
...
function main() {
  ...
  fail('pick a mode: --component <id> … or --starter (the code-bundle mode is gone — the app no longer self-updates; ship installers instead)');
}
