 ...
 * prints the exact commands to (1) upload the artifact to GitHub Releases and (2) commit
 * releases.json + trigger the catalog-indexer GitHub Action that regenerates manifest.json.
 *
...
function main() {
  ...
  // ── Step 2: commit releases.json + regenerate manifest.json via GitHub Actions (always printed) ──
  printDeploy();
  console.log('\n  (the indexer merges releases.json + upstream voices/languages → manifest.json,');
  console.log('   served from raw.githubusercontent.com/.../bookforge/catalog-data/manifest.json)\n');
