#!/bin/bash
# rubric-deploy — publish a rubric model and put it on every machine that runs it.
#
#   ./rubric-deploy.sh <version-tag> <merged-dir-or-f16-gguf> [quant] [win-host]
#   ./rubric-deploy.sh v4-4b ~/rubric-export/rubric-v4-f16-f16.gguf Q8_0
#
# rubric-publish.sh gets the artifact onto HuggingFace, which is what END USERS
...
# ~/rubric-export, and the Windows box in particular is the one where "did you
# test it on Windows?" keeps coming up.
#
#   1..5  everything rubric-publish.sh does (convert, quantize, load-check,
...
# instead of the copy. isRubricModelPresent() size-checks for the same reason.
#
# The catalog entry stays MANUAL, exactly as rubric-publish.sh argues: the sha256
...
TAG="${1:?usage: rubric-deploy.sh <version-tag> <merged-dir-or-f16-gguf> [quant] [win-host]}"
...
MAC_DIR="$HOME/Library/Application Support/OwenMorgan/rubric-models"
WIN_DIR='C:\Users\tellt\AppData\Local\OwenMorgan\rubric-models'

# ── 1-5. publish ─────────────────────────────────────────────────────────────
"$HERE/rubric-publish.sh" "$TAG" "$SRC" "$QUANT"

GGUF="$WORK/rubric-$TAG-$QUANT.gguf"
[ "$QUANT" = "f16" ] && GGUF="$WORK/rubric-$TAG-f16.gguf"
...
echo
echo "Still to do by hand: paste the catalog entry above into RUBRIC_MODELS in"
echo "electron/rubric-models.ts with a rank above the model it replaces, then"
echo "rebuild. Until that lands, the app will not offer rubric-$TAG."
