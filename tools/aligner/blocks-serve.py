...
The prompt is built by the CALLER (blocks-encoder.ts), never here. This
...
def load(adapter: str, max_seq_length: int) -> None:
    ...
    print(f"[blocks] loaded {adapter}", flush=True)
...
def classify(pages: list[dict], batch: int) -> list[str]:
    ...
    for start in range(0, len(pages), batch):
        ...
        print(f"[blocks]   {min(start + batch, len(pages))}/{len(pages)} pages",
              flush=True)
    return answers
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *a) -> None:
        print(f"[blocks] {fmt % a}", flush=True)
...
def main() -> int:
    ...
    print(f"[blocks] listening on :{args.port}", flush=True)
