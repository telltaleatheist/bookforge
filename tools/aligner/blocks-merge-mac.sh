#!/bin/bash
# blocks-merge-mac — fold a trained LoRA into the base model ON THE MAC and
...
# no `--quantize` — 1.2 GB is not worth a lossy step. Set BLOCKS_QUANT="" to
...
# raw:true. One implementation of the format, in blocks-encoder.ts.
...
# the same operation. `BLOCKS_BASE` overrides if a run ever needs it.
BASE_ID="${BLOCKS_BASE:-$(python3 - "$CKPT" <<'PY'
