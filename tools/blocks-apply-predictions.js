#!/usr/bin/env node
/**
 * blocks-apply-predictions — run a shipping blocks model over one corpus book and
 ...
 *
 *   node --require ./cli/electron-stub.js tools/blocks-apply-predictions.js \
 *     --book Twisted_Cross_-_Bergen,_Doris_L_\(1996\) --model blocks-v5 --backend ollama
 *
...
if (!bookDir || !model) {
  console.error('usage: node --require ./cli/electron-stub.js tools/blocks-apply-predictions.js \\\n' +
...
if (!fs.existsSync(labelsPath)) {
  console.error(`blocks-apply-predictions: no labels.json in ${srcDir}`);
  process.exit(1);
}
...
if (!labels.labels || typeof labels.labels !== 'object') {
  console.error(`blocks-apply-predictions: ${labelsPath} has no top-level "labels" map. ` +
...
const humanLabel = (b) => {
  ...
  if (v === undefined) {
    console.error(`blocks-apply-predictions: block ${b.id} (page ${b.page}) has no entry in ` +
...
if (wanted.length !== 1) {
  console.error(`blocks-apply-predictions: ${bookDir} matched ${wanted.length} SFT book slugs ` +
    `(${wanted.join(', ') || 'none'}). Candidates: ${[...slugs].join(', ')}`);
  process.exit(1);
}
const slug = wanted[0];
const mine = rows.filter(r => slugOf(r) === slug).sort((a, b) => a.page - b.page);
if (!mine.length) { console.error(`blocks-apply-predictions: no SFT rows for ${slug}`); process.exit(1); }
...
(async () => {
  ...
  for (let i = 0; i < mine.length; i += batch) {
    ...
    if (!res.success) { console.error(`\nblocks-apply-predictions: ${res.error}`); process.exit(1); }
    ...
    slice.forEach((r, k) => {
      ...
      if (declared !== pageBlocks.length) {
        console.error(`\nblocks-apply-predictions: page ${r.page} prompt declares ${declared} blocks ` +
