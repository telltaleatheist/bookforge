...
def main():
    ...
    def report(rs, label):
        ...
        false_edits = sum(1 for x in idents if x['touched'])
        ...
        return {
            ...
            'identityRows': len(idents), 'falseEdits': false_edits,
            'falseEditRate': false_edits / max(1, len(idents)),
            'exactMatch': exact / n,
    ...
    if args.json_out:
        ...
        with open(out, 'w') as fh:
            json.dump({'sft': path, 'endpoint': args.endpoint, 'guard': args.guard,
                       'overall': overall, 'perBook': per_book, 'rows': results}, fh, indent=1)
        print(f'\nwrote {out}')
