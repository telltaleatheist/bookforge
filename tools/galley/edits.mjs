/**
 * galley's edit contract — derive edits from a pair, apply them, and prove the
 * applier can never invent text.
 *
 *   node tools/galley/edits.mjs --self-test
 *
 * WHY THIS FILE EXISTS AT ALL. rubric labels a block and dagger deletes from
 * one; both are safe by construction — a wrong label narrates the wrong thing,
 * a wrong deletion drops a character. galley REWRITES prose that then becomes
 ...
 *
 * The format is dagger's, deliberately — one `before → after` per line, the
 ...
 *      wants to change, which is the same anchor discipline dagger proved: it
 ...
 *      block. This is the rule dagger did not need and galley cannot do
...
if (process.argv[1] && process.argv[1].endsWith('edits.mjs')) {
  ...
  console.error('usage: node tools/galley/edits.mjs --self-test');
  process.exit(1);
}
