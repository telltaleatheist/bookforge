#!/usr/bin/env node
/**
 * paragraph-audit — how much PARAGRAPH damage is actually in the galley corpus,
 * and how much of it galley could even be blamed for.
 *
 *   node tools/galley/paragraph-audit.mjs --pairs ~/Documents/BookForge/training/galley/paragraph-probe
 *
 * galley repairs a block of OCR text. The question this answers is whether the
 ...
 * fix it is inside the block (galley's job) or in the block next door (the
 * `continues` head planned for rubric, docs/RUBRIC_TRAINING.md §9c).
 *
...
for (const r of rows) {
  ...
  for (let i = 0; i + 1 < oL.length; i++) {
    ...
    // same hard hyphen the printer set, in which case it teaches galley
