/**
 * Exercise electron/blocks-run.ts without a model.
 *
 *   npm run build:electron   (or: npx tsc -p tsconfig.electron.json)
 *   node tools/test-blocks-run.js
 *
 * blocks-bridge is stubbed, so "classifying" is deterministic and instant and
...
const BRIDGE = path.join(DIST, 'blocks-bridge.js');
const RUN = path.join(DIST, 'blocks-run.js');
...
const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'blocks-run-test-'));
...
const base = {
  ...
  model: 'blocks-v3-4b', adapter: 'blocks-v3-4b', chunk: 4,
};
...
async function main() {
  console.log('\n1. a clean run answers every page exactly once');
  {
    ...
    m.blocksRunStart({ ...base, pages: pages(10) });
  ...
  console.log('\n2. a failure keeps the pages already answered');
  {
    ...
    const m = freshManager();
    m.blocksRunStart({ ...base, pages: pages(12) });
    const s = await settle(m, base.bookKey);
  ...
  console.log('\n3. restarting the SAME question resumes instead of re-asking');
  {
    ...
    const attached = m.blocksRunAttach(base.bookKey);
  ...
  console.log('\n4. a DIFFERENT question discards the old answers');
  {
    ...
    m.blocksRunStart({ ...base, pages: pages(12, '-v2') });
    const s = await settle(m, base.bookKey);
  ...
  console.log('\n5. cancel stops the run and keeps what landed');
  {
    ...
    fs.rmSync(stateDir, { recursive: true, force: true });
    const m = freshManager();
    m.blocksRunStart({ ...base, pages: pages(200) });
    await new Promise(r => setTimeout(r, 20));
    const res = m.blocksRunCancel(base.bookKey);
    ...
    check('runActive is false afterwards', m.blocksRunActive() === false);
  ...
  console.log('\n6. joining a live run does not restart it');
  {
    ...
    fs.rmSync(stateDir, { recursive: true, force: true });
    const m = freshManager();
    m.blocksRunStart({ ...base, pages: pages(100) });
    await new Promise(r => setTimeout(r, 20));
    const joined = m.blocksRunStart({ ...base, pages: pages(100) });
    ...
    const watched = m.blocksRunAttach(base.bookKey);
  ...
  console.log('\n7. a run with no pages is rejected, not silently empty');
  {
    const m = freshManager();
    let threw = false;
    try { m.blocksRunStart({ ...base, bookKey: 'empty', pages: [] }); }
    catch { threw = true; }
    check('threw', threw);
    let threwRaw = false;
    try {
      m.blocksRunStart({ ...base, bookKey: 'noraw',
