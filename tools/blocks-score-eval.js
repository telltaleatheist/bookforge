#!/usr/bin/env node
/**
 * blocks-score-eval — score a SHIPPING blocks model over a whole eval split.
 *
 *   node --require cli/electron-stub.js tools/blocks-score-eval.js \
 *     [--sft /Volumes/Callisto/training/rubric/sft/eval.jsonl] \
 *     [--model blocks-v3-4b] [--backend local|ollama] [--limit N] [--json out.json]
 *
 * WHY THIS EXISTS ALONGSIDE THE OTHER TWO SCORERS. tools/aligner/eval-blocks.py
 * scores an ADAPTER on the training box: it needs the GPU, unsloth, and the base
 * at the precision the run trained against. tools/blocks-replay.js scores the
 ...
 * eval-blocks.py: confusion keyed (truth, predicted) with `<missing>` and
...
if (argv.includes('--help') || argv.includes('-h')) {
  console.error(
    'usage: node --require cli/electron-stub.js tools/blocks-score-eval.js\n' +
...
if (!fs.existsSync(sftPath)) { console.error(`blocks-score-eval: no such file: ${sftPath}`); process.exit(1); }
...
if (!rows.length) { console.error('blocks-score-eval: no rows to score'); process.exit(1); }
...
(async () => {
  ...
  for (let i = 0; i < rows.length; i += batch) {
    ...
    const res = await blocksClassify({
      ...
      stop: enc.BLOCKS_STOP, numCtx: 12288,
      pages: slice.map(r => ({
    ...
    if (!res.success) { console.error(`\nblocks-score-eval: ${res.error}`); process.exit(1); }
  ...
  // its recall but belong to no class's precision — same as eval-blocks.py.
  const perClass = {};
