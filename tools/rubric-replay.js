...
function usage(msg) {
  if (msg) console.error(`rubric-replay: ${msg}`);
  console.error(
    'usage: node --require cli/electron-stub.js tools/rubric-replay.js --book <slug>\n' +
    '         [--sft <train-or-eval.jsonl>] [--model rubric-v3-4b] [--json out.json]\n' +
...
(async () => {
  ...
  for (let i = 0; i < rows.length; i += B) {
    ...
    const res = await rubricClassify({
      ...
      stop: enc.RUBRIC_STOP, numCtx: 8192,
      pages: slice.map(r => ({
    ...
    if (!res.success) { console.error(`rubric-replay: ${res.error}`); process.exit(1); }
    slice.forEach((r, k) => {
