/**
 * Does ocr's edit list survive the PRODUCTION applier?
 *
 * ocr's corpus was round-trip verified against tools/foundry-ocr/edits.mjs. But the
 ...
 * word-boundary lookarounds. If those guards reject ocr's true corrections,
 ...
 * docs/FOUNDRY_OCR_INTEGRATION.md §1. Re-run this after any change to either contract.
 *
 * usage: node tools/foundry-ocr/contract-crosscheck.mjs <sft.jsonl...>
...
if (!files.length) {
  console.error('usage: node tools/foundry-ocr/contract-crosscheck.mjs <sft.jsonl...>');
  process.exit(1);
}
...
let ocrOk = 0, prodExact = 0, prodDiffered = 0;
...
for (const f of files) {
  for (const line of fs.readFileSync(f, 'utf8').split('\n')) {
    ...
    // ocr's own applier — the corpus guarantee
...
console.log(`ocr applier accepted rows:      ${ocrOk}/${editRows}`);
