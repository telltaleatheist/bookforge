/**
 * build-corpus — turn mined OCR↔truth pairs into ocr's SFT split.
 *
 *   node tools/foundry-ocr/build-corpus.mjs \
 ...
 *
 * Every row is round-trip verified by tools/foundry-ocr/edits.mjs before it is
...
if (argv.includes('--help') || !argv.length) {
  console.error('usage: node tools/foundry-ocr/build-corpus.mjs --pairs <a.jsonl,b.jsonl> [--stats <dir>]\n' +
...
if (ungated.length) {
  ...
  console.error('Mine them with tools/foundry-ocr/mine-book.mjs, which writes the stats the gate reads.');
  process.exit(1);
}

/**
 * THE AUTHORITATIVE GATE is text-quality.py's verdict on the truth we actually
 * mined (tools/foundry-ocr/gate-mined-truth.mjs). Two earlier candidates each got a
...
if (!fs.existsSync(truthQualityPath)) {
  console.error(`build-corpus: no mined-truth-quality.json in ${statsDir}.\n` +
    'Run: node tools/foundry-ocr/gate-mined-truth.mjs --pairs ' + statsDir + '\n' +
...
// ── report ──────────────────────────────────────────────────────────────────
console.log(`ocr build-corpus — ${rows.length} mined pairs from ${pairFiles.length} file(s)`);
