 ...
 *
 *   node tools/foundry-ocr/degrade-batch.mjs \
...
if (argv.includes('--help') || !argv.length) {
  console.error('usage: node tools/foundry-ocr/degrade-batch.mjs --books a,b,c --levels speckle0.8,combo-mild\n' +
    '         [--pairs <dir>] [--work <dir>] [--concurrency 2] [--python python3] [--force] [--keep-pdf]\n' +
    '       levels come from tools/foundry-ocr/degrade-render.py --list');
...
if (!levels.length) die('--levels is required; see `python3 tools/foundry-ocr/degrade-render.py --list`');
...
for (const j of jobs) {
  ...
  try {
    ...
    await run('node', ['--require', path.join(REPO, 'cli/electron-stub.js'),
      ...
      { OCR_OCR_CONCURRENCY: String(concurrency) });
  ...
  st.degradation = { level: j.level, renderer: 'tools/foundry-ocr/degrade-render.py', dpi: 200, pageOffset: j.from };
  st.cleanBaseline = {
