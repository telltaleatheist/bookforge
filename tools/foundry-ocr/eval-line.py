#!/usr/bin/env python3
"""
eval-line — score the ocr LINE corrector on its held-out books.

    python3 tools/foundry-ocr/eval-line.py \
        [--sft /Volumes/Callisto/training/rubric/ocr/sft-line/eval.jsonl] \
...
# Mirrored from tools/foundry-ocr/build-dataset.py and tools/foundry-ocr/score.js — the
...
def qwen3_prompt(system, user):
    ...
    `<|im_start|>assistant\\n` and feeds the model a shape it never saw. blocks
    lost time to exactly this (see CLAUDE.md, "Blocks — the page-layout model"),
...
def main():
    ...
    ap.add_argument('--sft', default='/Volumes/Callisto/training/rubric/ocr/sft-line/eval.jsonl')
    ...
    ap.add_argument('--german', default='/Volumes/Callisto/training/rubric/ocr/sft-line/eval-german.jsonl',
