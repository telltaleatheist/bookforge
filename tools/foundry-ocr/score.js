#!/usr/bin/env node
/**
 * ocr-score — score the OCR corrector on its held-out books.
 *
 *   node --require ./cli/electron-stub.js tools/foundry-ocr/score.js \
 *     [--sft ~/Documents/BookForge/training/galley/sft/eval.jsonl] \
 *     [--model foundry-ocr-v1-4b] [--backend ollama|local] [--limit N] [--json out.json]
 *
 ...
 * average, every time.
 *
 * `applier rejected` is not a damage number — the contract in tools/foundry-ocr/edits.mjs
...
if (argv.includes('--help') || argv.includes('-h')) {
  console.error('usage: node --require ./cli/electron-stub.js tools/foundry-ocr/score.js\n' +
    '         [--sft <eval.jsonl>] [--model foundry-ocr-v1-4b] [--backend ollama|local]\n' +
...
if (!fs.existsSync(sftPath)) { console.error(`ocr-score: no such file: ${sftPath}`); process.exit(1); }
const model = opt('model', 'foundry-ocr-v1-4b');
...
(async () => {
  ...
  const { parseEdits, applyEdits, formatEdits } = await import('./ocr/edits.mjs');
  const { loadBlocksEncoder } = require(path.join(REPO_ROOT, 'cli/lib/load-blocks-encoder.js'));
  ...
  if (!rows.length) { console.error('ocr-score: eval set is empty'); process.exit(1); }
  ...
  const cases = rows.map((r, i) => {
    ...
    if (!applied.ok) {
      console.error(`ocr-score: gold row ${i} (${r.book} p${r.page}) does not satisfy the applier ` +
        `contract — the corpus and tools/foundry-ocr/edits.mjs have drifted. Rebuild the corpus.`);
      process.exit(1);
    }
  ...
  console.log(`ocr-score — ${cases.length} blocks from ${sftPath}`);
  ...
  for (let i = 0; i < cases.length; i += batch) {
    ...
    if (!res.success) { console.error(`\nocr-score: ${res.error}`); process.exit(1); }
    slice.forEach((c, k) => {
      const answer = res.answers[k];
      if (typeof answer !== 'string') {
        console.error(`\nocr-score: backend returned no answer for ${c.r.book} p${c.r.page}`);
        process.exit(1);
      }
      results.push({ ...c, answer });
    });
    const secs = (Date.now() - started) / 1000;
    process.stderr.write(`\r[ocr-score] ${results.length}/${cases.length}  ` +
