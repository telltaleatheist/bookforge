/**
 * End-to-end check of the BUILT-IN blocks runtime.
 *
 *   npm run build:electron
 *   npx tsc src/app/features/pdf-picker/services/blocks-encoder.ts \
 *     --outDir dist/blocks --module commonjs --target es2022 --skipLibCheck
 *   node --require cli/electron-stub.js tools/test-blocks-local.js [blocks.json]
 *
 * Needs the model installed (Settings -> Add-ons, or tools/aligner/blocks-publish.sh's
...
const enc = (() => {
  for (const p of ['dist/blocks/features/pdf-picker/services/blocks-encoder.js',
                   'dist/blocks/blocks-encoder.js']) {
    try { return require(path.join(REPO_LOCAL, p)); } catch { /* next */ }
  }
  console.error('The encoder is not compiled. Run:\n  npx tsc ' +
    'src/app/features/pdf-picker/services/blocks-encoder.ts --outDir dist/blocks ' +
    '--module commonjs --target es2022 --skipLibCheck');
  process.exit(1);
})();
const { blocksClassify, blocksHealth, blocksModels } = require(path.join(REPO, 'dist/electron/blocks-bridge.js'));
const fs = require('fs');

const BLOCKS = process.argv[2]
  || '/Volumes/Callisto/training/rubric/unspeakable-truths/blocks.json';
const MODEL = 'blocks-v3-4b';
...
async function main() {
  ...
  const unknown = await blocksHealth('', 'local', 'blocks-nope');
  ...
  const res = await blocksClassify({
    backend: 'local', model: MODEL, endpoint: '', stop: enc.BLOCKS_STOP,
  ...
  const legal = new Set(enc.blocksCategories(version));
  ...
  const t2 = Date.now();
  const res2 = await blocksClassify({
    backend: 'local', model: MODEL, endpoint: '', stop: enc.BLOCKS_STOP,
  ...
  const bad = await blocksClassify({
  ...
  const { stopBlocksServer } = require(path.join(REPO, 'dist/electron/blocks-server.js'));
