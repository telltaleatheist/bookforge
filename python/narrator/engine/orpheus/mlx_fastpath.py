...
Ported VERBATIM from ebook2audiobook@9daab0ba
lib/classes/tts_engines/orpheus_mlx_fastpath.py. The only edits are the path
references in the prose (orpheus.py -> narrator.engine.snac /
narrator.engine.adapters / narrator.engine.mlx_backend) and non-ASCII
punctuation replaced with ASCII per the CONTRACTS rule; every line of CODE
is unchanged.
...
2. THE DEAD 82% OF THE LM HEAD. Orpheus can only ever emit END_OF_AUDIO
   ...
   in narrator.engine.snac keeps exactly that range and throws the rest away. The stock
...
END_OF_AUDIO). The MLX backend never reads them - both MLX batch call sites use only
...
an `nn.Embedding`. The MLX adapter path (`AdaptersMixin._mlx_adapter_plan`) refuses by
...
# Orpheus token geography (config.END_OF_AUDIO_TOKEN, snac._redistribute_codes).
...
def make_eos_boost(base: float, start: float, expected: float,
    ...
    """The `_boost` closure of `SamplingMixin._mlx_eos_boost_processor`, marked.
    ...
    for proc in procs:
        ...
        if mark is None:
            raise FastPathUnsupported(
                ...
                'narrator.engine.mlx_fastpath.make_rep_penalty / make_eos_boost; build it '
...
def install(model, *, rep_window: int, max_tokens: int) -> str:
    ...
    here (mlx_backend.py owns ORPHEUS_MLX_FASTPATH).
    """
    import mlx.nn as nn
    import mlx_lm
