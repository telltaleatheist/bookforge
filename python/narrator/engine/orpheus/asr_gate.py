...
lib/classes/tts_engines/common/asr_gate.py, with one import rewired:
`_big_num_words` now comes from narrator.engine.text (which holds the live half
of e2a's orpheus_text.py) instead of from lib.classes.tts_engines.common.
...
narrator.engine.text picks those chunks); this module is the check itself.
...
('KUHN' for 'Kun') stays below the word-run thresholds.
...
def _norm_words(text: str) -> list:
    ...
    from .text import _big_num_words
