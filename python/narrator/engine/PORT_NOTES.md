...
| half | state |
|---|---|
| (a) **the server's own log, READ** - every trailing-ramp line reports exactly 2 frames (the count-before-trim instrumentation), and ZERO sync-path interior drops | **DONE 2026-09-05** - `verify_sentinel_filter`, run by `load_engine` right after the probe render. `start()` no longer sends the launcher's streams to DEVNULL: both go to a file the backend owns, `<process_dir>/higgs-v3-server.log` when there is a session (else a per-instance file beside the pid file), overwritten per start. The path rides on `BackendSpec.server_log` and in the `loaded` message as `serverLog`, so a tool or a ledger entry can re-run the proof. ATTACH mode reads a log only when the operator names one in `NARRATOR_HIGGS3_SERVER_LOG` - no default, because a stale log would let the proof pass on a server that is no longer up - and `load_engine` says UNAVAILABLE out loud when there is none |
...
| 6 | ~~**The tail-trim probe's -45 dBFS gate.**~~ RETIRED 2026-09-05, and not by measuring it: `patch_sentinel_filter.py` REMOVES the sentinel frames, so the window holds the model's own audio and the certifying box read -35..-38 dBFS on BOTH builds. No level separates them. `probe_sentinel_filter` reports against that band and gates on nothing. ~~**TODO(higgs-sentinel-proof)**~~ **CLOSED 2026-09-05**: the patch's real proof is (a) `verify_sentinel_filter`, which reads the server log narrator now owns, and (b) `[:, :-1]` absent from the stage processor, which the BookForge doctor greps for. What remains unmeasured here is only the count itself: 2 frames per line is what the certifying box observed, and a line reporting any other count has never been seen - the refusal is written against a shape nobody has produced | render against a server carrying the queued count-before-trim fix, which should print 0 rather than 2 and will need its own certificate |
