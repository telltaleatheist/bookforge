 ...
 * llama-model-server.ts, shared with the footnote-marker model. What is rubric's
 * own is here: which catalog it reads, how much context a page needs, and its
 * arbiter identity.
 */
...
export type RubricGenerateResult = LlamaGenerateResult;

export const rubricServer = new LlamaModelServer({
  logTag: 'rubric',
  modelLabel: 'page-layout model',
  port: PORT,
  contextSize: CONTEXT,
  neededVramMB: NEEDED_VRAM_MB,
  gpuOwner: GPU_OWNER_RUBRIC,
  ...
  resolveModel: (modelId) => {
    ...
    return {
      name: def.name,
      path: rubricModelPath(modelId),
      present: isRubricModelPresent(modelId),
    };
  },
});
