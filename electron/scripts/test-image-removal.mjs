 ...
 *
 * Run with: node electron/scripts/test-image-removal.mjs
 */
...
async function main() {
  ...
  // Helper to format bbox (could be array or object)
  const formatBbox = (bbox) => {
    if (Array.isArray(bbox)) {
      return bbox.map((n) => n.toFixed(1)).join(', ');
    } else if (bbox && typeof bbox === 'object') {
      return `x:${bbox.x?.toFixed(1) || '?'}, y:${bbox.y?.toFixed(1) || '?'}, w:${bbox.w?.toFixed(1) || '?'}, h:${bbox.h?.toFixed(1) || '?'}`;
    }
  ...
  console.log('\n=== Page Structure ===');
  console.log('Raw first block:', JSON.stringify(stextJson.blocks[0], null, 2).substring(0, 500));

  for (const block of stextJson.blocks) {
    if (block.type === 'image') {
      console.log(`IMAGE: bbox=[${formatBbox(block.bbox)}]`);
    } else if (block.type === 'text') {
      const lines = block.lines || [];
      const firstLine = lines[0];
      if (firstLine) {
        const text = firstLine.spans?.map((s) => s.text).join('') || '';
        console.log(`TEXT: "${text.substring(0, 50)}..." bbox=[${formatBbox(block.bbox)}]`);
      }
    }
  }

  // Find the first image block
  const imageBlock = stextJson.blocks.find((b) => b.type === 'image');
  if (!imageBlock) {
  ...
  const imageBbox = bboxToArray(imageBlock.bbox);
  console.log(`Target image bbox: [${imageBbox.map((n) => n.toFixed(1)).join(', ')}]`);
  ...
  await testRedactionPreserveText(mupdf, TEST_PDF, pageNum, imageBbox, `${OUTPUT_DIR}/02-redact-preserve-text.png`, `${OUTPUT_DIR}/02-redact-preserve-text.pdf`);

  // Test 3: Use removeWithOverlay (Square annotation + bake)
  console.log('\nTest 3: removeWithOverlay (Square annotation + bake)...');
  await testOverlayMethod(mupdf, TEST_PDF, pageNum, imageBbox, `${OUTPUT_DIR}/03-overlay-bake.png`, `${OUTPUT_DIR}/03-overlay-bake.pdf`);

  // Test 4: Pixel-level painting only (no PDF modification)
  console.log('\nTest 4: Pixel-level painting (render then paint over)...');
  await testPixelPainting(mupdf, TEST_PDF, pageNum, imageBbox, `${OUTPUT_DIR}/04-pixel-paint.png`);
  ...
  await testRedactionRemoveAll(mupdf, TEST_PDF, pageNum, imageBbox, `${OUTPUT_DIR}/05-redact-remove-all.png`, `${OUTPUT_DIR}/05-redact-remove-all.pdf`);
