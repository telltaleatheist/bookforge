 ...
 *
 * Run with: npx ts-node --esm electron/scripts/test-image-removal.ts
 */
...
async function main() {
  ...
  // Check page 1 for images
  const pageNum = 0;
  const page = pdfDoc.loadPage(pageNum) as InstanceType<typeof mupdf.PDFPage>;
  const bounds = page.getBounds();
  console.log(`\nPage ${pageNum + 1} bounds: [${bounds.join(', ')}]`);

  // Find images on the page by examining the page content
  // Get the structured text to understand the page content
  ...
  for (const block of stextJson.blocks) {
    if (block.type === 'image') {
      console.log(`IMAGE: bbox=[${block.bbox.map((n: number) => n.toFixed(1)).join(', ')}]`);
    } else if (block.type === 'text') {
      const lines = block.lines || [];
      const firstLine = lines[0];
      if (firstLine) {
        const text = firstLine.spans?.map((s: any) => s.text).join('') || '';
        console.log(`TEXT: "${text.substring(0, 50)}..." bbox=[${block.bbox.map((n: number) => n.toFixed(1)).join(', ')}]`);
      }
    }
  }

  // Find the first image block
  const imageBlock = stextJson.blocks.find((b: any) => b.type === 'image');
  if (!imageBlock) {
  ...
  console.log(`Target image bbox: [${imageBlock.bbox.map((n: number) => n.toFixed(1)).join(', ')}]`);
  ...
  await testRedactionPreserveText(mupdf, TEST_PDF, pageNum, imageBlock.bbox, `${OUTPUT_DIR}/02-redact-preserve-text.png`, `${OUTPUT_DIR}/02-redact-preserve-text.pdf`);

  // Test 3: Use removeWithOverlay (Square annotation + bake)
  console.log('\nTest 3: removeWithOverlay (Square annotation + bake)...');
  await testOverlayMethod(mupdf, TEST_PDF, pageNum, imageBlock.bbox, `${OUTPUT_DIR}/03-overlay-bake.png`, `${OUTPUT_DIR}/03-overlay-bake.pdf`);

  // Test 4: Pixel-level painting only (no PDF modification)
  console.log('\nTest 4: Pixel-level painting (render then paint over)...');
  await testPixelPainting(mupdf, TEST_PDF, pageNum, imageBlock.bbox, `${OUTPUT_DIR}/04-pixel-paint.png`);

  // Test 5: Use applyRedactions with all methods (the problematic case?)
  console.log('\nTest 5: applyRedactions (image_method=2, text_method=0 - remove text too)...');
  await testRedactionRemoveAll(mupdf, TEST_PDF, pageNum, imageBlock.bbox, `${OUTPUT_DIR}/05-redact-remove-all.png`, `${OUTPUT_DIR}/05-redact-remove-all.pdf`);
...
async function savePageAsPng(mupdf: typeof import('mupdf'), pdfPath: string, pageNum: number, outPath: string) {
...
async function testRedactionPreserveText(
  ...
  const rect: [number, number, number, number] = [bbox[0], bbox[1], bbox[2], bbox[3]];
...
async function testOverlayMethod(
  mupdf: typeof import('mupdf'),
  pdfPath: string,
  pageNum: number,
  bbox: number[],
...
async function testPixelPainting(
  mupdf: typeof import('mupdf'),
  pdfPath: string,
  pageNum: number,
  bbox: number[],
...
async function testRedactionRemoveAll(
  mupdf: typeof import('mupdf'),
  pdfPath: string,
  pageNum: number,
  bbox: number[],
