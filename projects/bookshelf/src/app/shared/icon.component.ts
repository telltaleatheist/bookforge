...
export class IconComponent {
  ...
  private static readonly PATHS: Record<string, string> = {
    ...
    // Hourglass — the queue button. The desktop app's nav rail spells "Queue"
    // with an hourglass, so the web wears the same identity. NOT `list`: three
    // horizontal bars beside the server-rack hamburger read as a second menu
    // (Owen, 2026-08-27).
    ...
    // Server rack (stacked storage bars) — the "Libraries" menu (servers + device).
    ...
    // Pencil — the per-bookmark rename affordance.
    ...
    // Framed picture with mountains — the "book cover" view toggle.
