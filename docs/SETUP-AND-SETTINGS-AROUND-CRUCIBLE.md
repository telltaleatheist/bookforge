...
| 2. Use the one on this machine (report + Test it) | **KEEP-REWORD** | ruling 0h (2026-09-15): the door ADDS the engine found on this computer, under a name the operator types; once it is a row it is **Open Crucible** |
...
| *(absent)* probe on entry | **RULING → BUILD** | §0b C2: the step should PROBE on entry — a server registered → connected; none registered but one found here → Adopt (name + Add); none and hostable → Install; not hostable → Connect only. Today all three doors are always offered, closed |
...
| `wslDistro` | which WSL distro | `tool-paths.json` | `tool-paths.ts:2108` → **including `electron/crucible/install.ts:340,402,412` and `crucible/servers.ts:480`** | **KEEP** | the ONE WSL key with a non-legacy reader: `discovery.ts` reads a Crucible on this computer out of `config.toml` through `wsl.exe -d <distro>`. It should MOVE to the Crucible section |
...
| option | what it does today | value lives | who reads it | DISP | why |
|---|---|---|---|---|---|
| ~~"This machine" card~~ + Re-check | **DELETED by ruling 0h (2026-09-15)**: there is one list and one kind of row. What survives is the OFFER — `serversView().discovered`, `electron/crucible/discovery.ts` — which prefills the add door | not persisted, by design | the registry | **CHANGED** | a server here is a registry entry like any other |
...
| door 2 (Add the one on this computer) | reports name/url/configPath, and ADDS it under a name you type | — | `crucible/discovery.ts` | **CHANGED** | ruling 0h (2026-09-15): this door ADDS the engine found here; once it is a row it is **Open Crucible** |
...
- **a server is registered** → *Connected*, the names it knows, and one button,
...
| door 2, this machine | KEEP-REWORD | once the engine here is a registered row it is **Open Crucible** plus **Set up for BookForge**, with Test beside them. |
...
| §3.13 "This machine" card | KEEP | KEEP, and it gained a SECOND door | the pairing file (crucible §3.6) is read before `config.toml`; `CrucibleDiscoveryVia` is `'pairing' \| 'file' \| 'wsl'` and the row can still say which answered. The `wsl` door is dated by the contract and dies when the Windows host lands. |
...
|---|---|
| **"Set up for BookForge"**, a button on every server row and on both local faces | **DELETED.** `electron/crucible/coordinate.ts` is the one owner of "make sure this server has what BookForge needs", called on every CONNECT: app start for EVERY enabled server (ruling 0h), a server added, a server switched back on, the wizard's step landing on connected, and after a driven install (routed through the same function, not its own post). The row draws the STATE where the button was. |
