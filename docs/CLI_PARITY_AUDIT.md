...
| 1.13 | Listen / stream on Higgs | `tts-api-server.handleSpeak` binds a Higgs voice (shipped 2026-09-05); the engine is the persisted selection | `--tts --mode streaming --engine higgs` — **the wrapper's refusal was lifted 2026-09-12** | **Yes** | **PARITY** — `--engine` ASSERTS the persisted selection (`getSelectedEngineName`) and refuses a mismatch by name; it does not call `setSelectedEngineName`, which would rewrite the user's `tts-engine.json` |
