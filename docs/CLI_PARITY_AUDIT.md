...
| 1.5 | Listen / stream a page (extension) | ~~`tts-api-server.ts`~~ — **BookForge has no door here any more.** The extension is a Crucible client (Phase 16 step 8): it opens its own `POST /v1/tts/stream` session on the server it selected | ~~`--tts --mode streaming`~~ — deleted with the relay; refused by name | **n/a** | **NO DOOR EITHER SIDE** — parity is not the question once neither program owns the action. The CLI's Crucible verbs are the operator's door to that server |
| 1.6 | Render a Higgs book | same as 1.1, routed by `isHiggsJob` | `--tts --engine higgs` | same divergence as 1.1 | **DIVERGENT** |
| 1.7 | Render a Higgs CHECKPOINT under test (not a catalog voice) | no app door — the modal offers catalog voices only | **`--checkpoint-dir` (new, 2026-09-12)** → `ParallelTtsSettings.higgsOverride.checkpointDir`, resolved by the bridge against `--voice`'s certificate | **Yes** — the same `renderRangeHeadless`, one more settings field | **PARITY (CLI-only door)** — deliberately: a checkpoint under test is a dev act, and putting it in the modal would offer users a voice with no measured caps |
| 1.8 | Override Higgs sampling / caps / band for one run | no app door — a voice's numbers are the catalog's, measured | **`--temperature`/`--top-p`/`--top-k`/`--max-chars`/`--safe-band` (new)** → `higgsOverride.sampling` / `maxChars` / `safeMin·MaxChars` | **Yes** | **PARITY (CLI-only door)** — `note` is REQUIRED on the override, so a session's sampling is always attributable |
| 1.9 | Override Orpheus sampling for one run | the worker's `ORPHEUS_*` env seams | `--temperature`/`--top-p`/`--min-p`/`--rep-penalty`/`--max-chars` → the same env | **Yes** | **PARITY** — and the ORPHEUS_* seams are now set ONLY on `--engine orpheus`; a Higgs run would have nobody reading them |
| 1.10 | Tune the Mac MLX arm's group width / memory budget | `higgs-spawn.ts:399 higgsMlxBatchEnv`, which honours `process.env` over the catalog ceiling | **`--batch-width` / `--mem-budget-gb` (new)** → `NARRATOR_HIGGS3_MLX_BATCH` / `NARRATOR_HIGGS3_MLX_MEM_BUDGET_GB` | **Yes** — the same env the bridge reads | **PARITY (Mac only)** — refused by name on win32, where the width is the SERVER's admission width (`HIGGS_MAX_NUM_SEQS`), not a per-run knob |
| 1.11 | Render loose text / a set of test chunks | no app door — the app narrates projects | **`--input *.txt/.md/.jsonl`, `--text`, `--as-chunks` (new)** → packed into a one-chapter EPUB by `epub-writer.buildEpubBuffer`, the app's OWN writer; `--as-chunks` → `settings.sentencePerParagraph` | **Yes** — the render path still reads one format, and the book is the book the app writes | **PARITY (CLI-only door)** — Owen overrode the 2026-09-05 EPUB-only ruling on 2026-09-12 |
| 1.12 | Cap a render at N chunks | `ParallelTtsSettings.testMode` + `testSentences` (the app's own settings pair) | **`--max-chunks` (new)**, `--tts` only | **Yes** — the bridge applies the cap; the CLI does not slice the input | **PARITY** — refused on `--audiobook`, where a capped book would be filed as the project's own |
| 1.13 | Listen / stream on Higgs, IN THE APP | the Streaming tab: `stream:start` → `stream-scheduler` → `streaming-engine`; and `reader-stream-bridge.ts` for the phone. (`tts-api-server.handleSpeak` was the third and is deleted) | none — the adapter that drove it is gone | **No** | **CLI GAP, STATED** — the CLI has no headless drive of the app's in-process Listen path since 2026-09-15. Nothing was repointed at `streamScheduler` to fill it: that would be a door no app surface uses, which is the opposite of "the CLI mirrors the app's code path" |
