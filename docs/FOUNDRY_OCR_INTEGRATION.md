# foundry ocr → the app: integration plan

**Status: HISTORICAL PLAN.** Written Jul 31 2026, when the OCR corrector was
called *galley* and BookForge was going to host and run it itself. It did not
land that way: the model became foundry's `ocr` stage
(`foundry-ocr-v1-4b`), and BookForge now invokes it through the
`ocr-correction` processing pass rather than serving it — see
`PROCESSING_PIPELINE_V2.md`. Kept for the measurement and the reasoning, which
still hold; the file-by-file wiring below describes a seam that was never built.

The instruction (Jul 31 2026): *integrate it the same way the footnote-marker
model was integrated — except nothing gets commented out, ocr replaces the old
method completely. The old method used smaller local models.*
...
behind them are in `BLOCKS_TRAINING.md` §12; this document is only about the seam
...
general model" and ocr would be a different unit and a different failure mode.
...
So ocr is not introducing the edit-list idea. It replaces the **model** inside a
pipeline shaped for it. The unit is close too: production chunks run 1400–1700 chars
(≈350–450 tokens) against ocr's trained median of 188 and max of 1204 — in range,
...
`tools/foundry-ocr/contract-crosscheck.mjs` runs every **gold** edit in ocr's corpus
...
```
ocr applier accepted rows:      4508/4508     ← the corpus guarantee holds
...
**The production applier lands 18.6% of ocr's true corrections and blocks 81.4%.**

Piping ocr through it would produce a model that scores well in `ocr-score.js`
...
1. **72.5% of ocr's anchors sit mid-word.** `deriveEdits` widens an anchor
...
2. **The remaining guards encode "this model cannot be trusted with numbers, quotes,
   ...
   asked to repair prose. But it is exactly ocr's job description:
...
| | production `applyEditList` | ocr `edits.mjs` |
|---|---|---|
...
ocr's residual risk — a confidently wrong substitution the structure cannot catch —
is real, and it is precisely what the **false-edit rate** in `ocr-score.js`
...
- `LETTER_DELETION_BLOCKED` — a letter-bearing `find` replaced by blank. ocr never
  does this; keeping it costs nothing and closes the prose-deletion hole permanently.
- `SUSPICIOUS_GLOBAL` — subsumed by ocr's stricter uniqueness requirement.

The other eight must go **for the ocr path**. Record why in the code, so nobody
...
it goes too, because ocr's block-level budget already bounds it.
...
pass 1  runOcrRepair   chunk loop → cleanChunkEditList → repaired.epub   ← OCR REPLACES THE MODEL HERE
...
**Deleted, not commented out** (per instruction), once ocr ships:
...
> **None of the files in this table exist.** The four `dagger-*.ts` modules on
> the right were deleted in Aug 2026 along with the rest of that model's
> distribution, and the four `ocr-*.ts` modules on the left were never written —
> foundry hosts and serves the OCR corrector. Read this as the shape that was
> proposed, not as a map of the tree.

dagger was ~700 lines across four files. ocr was going to be the same shape.

| New file | Mirrors | Contents |
|---|---|---|
| `electron/ocr-models.ts` | `dagger-models.ts` (265 ln) | catalog: HF resolve URL, sha256, bytes, rank; `bestInstalledOCRModel()`, download/delete |
| `electron/ocr-server.ts` | `dagger-server.ts` (80 ln) | `LlamaModelServer`, **port 8771**, `GPU_OWNER_OCR = 'llama:ocr'`, `/completion` only |
| `electron/ocr-repair.ts` | `dagger-footnotes.ts` (275 ln) | prompt build, arrow-list parse, apply, per-edit disposition records |
| `electron/components/ocr-model-components.ts` | `dagger-model-components.ts` (58 ln) | Add-ons + first-run entry |

Ports in use: 8766 tts-api, 8769 blocks/llama-bridge, 8770 dagger → **8771 ocr**.

### 3a. One contract implementation, three consumers

`tools/foundry-ocr/edits.mjs` is currently the trainer's and the scorer's. The app needs
it too, and a second copy would drift — exactly the mistake `blocks-encoder.ts`
avoids by being THE prompt format with main never parsing answers.

**Port it to `shared/text/ocr-edits.ts`** and have `tools/foundry-ocr/edits.mjs`
...
`build-corpus.mjs`, and — like blocks — the prompt goes to `/completion` **verbatim**
...
ocr trained on **blocks** (median 188 tokens). Production feeds **chunks** of
1400–1700 chars. Length alone is fine — a chunk is ≈350–450 tokens, well inside the
1204 max.

The risk is not length, it is **uniqueness**. ocr learned to widen an anchor until
...
Cheap to settle: run `ocr-score.js` twice on the held-out books, once at block
granularity and once with blocks concatenated to production chunk size, and compare
CER reduction and rejection rate. If chunking hurts, feed ocr paragraph-sized
...
**Do this before writing `ocr-repair.ts`**, since it decides that file's unit.
...
1. **Train `ocr_v1_4b`.** Profiles in `tools/foundry-ocr/training-profiles.json`.
   **Never start a run without an explicit green light** — shared GPU, faulty fan.
   Then the 0.6B control.
2. **Score** with `tools/foundry-ocr/score.js` on the 7 held-out books. Read
...
4. **Publish** the GGUF at Q8_0 — Q4_K_M cost more than the whole v3→v4 gain on
   blocks, and this task is near-verbatim string copying, where quantization hurts
   most. Catalog entry into `ocr-models.ts`.
5. **Ship the contract** to `shared/text/ocr-edits.ts`; re-point the tools at it.
6. **Server + catalog + Add-ons component.** ocr downloadable, startable,
   GPU-arbitrated alongside blocks and dagger.
